...
class Position:
    ...
    def __init__(self, position=None, boundaries=1, acceleration_limit=0.01, nearness_threshold = 0.1, drag = [1.0, 0.7, 1.0]):
        ...
        if not position:
            ...
            self.position = [[random.random() if deriv_level == 0 else 0 for _ in range(2)] for deriv_level in range(3)]
    ...
    def accelerate_in_direction(self, acceleration):
        ...
        for derivative_level in range(len(self.position) - 2, -1, -1):
            for dim in range(len(self.position[0])):
                ...
                self.position[derivative_level][dim] *= 1.0 if derivative_level>= len(self.drag) else self.drag[derivative_level]
        self.bounce()
    ...
    def bounce(self):
        while self._single_bounce():
            pass
        assert(all([self.position[0][x] >= 0 for x in range(len(self.position[0])-1)]))
    ...
    def _reflect(self, dim, low):
        if low:
            ...
            self.position[0][dim] -= 2*(self.position[0][dim] - self.boundaries)
...
class CatMouseSimulator:
    ...
    """
    def iterate(self):
        closest_mice = [argmin(self.mice, lambda mouse: dist(mouse.position[0], cat.position[0])) for cat in self.cats]
        direction_to_mice = [[closest_mice[i][0].position[0][d] - cat.position[0][d] for d in range(len(cat.position[0]))] for i, cat in enumerate(self.cats)]
        mice_moves = [self.cat_behavior.react(self.cats, self.mice[i].position[0]) for i in range(len(self.mice))]
        # Move
        ...
        # Kill
        kill_indices = [self.mice[i].is_near(self.cats) for i in range(len(self.mice))]
        self.killed.extend([self.mice[i] for i in range(len(self.mice)) if kill_indices[i]])
        self.mice = [self.mice[i] for i in range(len(self.mice)) if not kill_indices[i]]
        # Optionally, plot
        if self.plotter:
...
class CatMouseVisualizer:
    ...
    def visualize_locations(self, cats, mice, killed):
        self.imgs.append(Image.new('RGB', (self.img_size, self.img_size), "black"))
        draw = ImageDraw.Draw(self.imgs[-1])
        dot_size = self.img_size*Position().nearness_threshold/2
        def draw_at(pos, color):
            draw.ellipse((pos[0]*self.img_size-dot_size, pos[1]*self.img_size-dot_size, pos[0]*self.img_size+dot_size, pos[1]*self.img_size+dot_size), fill=color)
    ...
    def complete(self):
        ...
        Image.new('RGB', (self.img_size, self.img_size), "black").save(fp=self.target, format='GIF', append_images=self.imgs, save_all=True, duration=20, loop=0)

def main():
    vis = CatMouseVisualizer()
    iters = simulate_cat_mouse(3, 50, Behavior([0]*4), vis.visualize_locations, vis.complete)
